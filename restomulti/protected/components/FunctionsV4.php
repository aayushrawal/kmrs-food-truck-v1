...
/*	public static function getItemFirstPrice($prices='',$discount='')
    ...
    public static function reCheckDelivery($merchant_id='',$data='')
    {
    	...
    	if($merchant_info=FunctionsV4::getMerchantById($merchant_id)){
	...
	public static function saveFbAvatarPicture($id='')
	{
		...
		if(isset($image['data']['url'])){
			...
			@file_put_contents( FunctionsV4::uploadPath()."/$fb_avatar_filename",$image);
    ...
    public static function replaceTemplateTags($tpl='',$pattern='', $data='')
    {
    	foreach ($pattern as $key=>$val) {
    		switch ($key) {
    			...
    			case "total_amount":
    			    $tpl=self::smarty($key,
    				isset($data[$val])?FunctionsV4::prettyPrice($data[$val]):'',$tpl);
    			    break; 
    	...
    	if ($data=Yii::app()->functions->getOrder2($order_id)){       		    		
	    	...
	    	if (FunctionsV4::hasModuleAddon("mobileapp")){
		    	...
		    	if($enabled==1 && !empty($tpl) && is_array($client_info) && count($client_info)>=1){
			    	...
			    	FunctionsV4::fastRequest(
			    	FunctionsV4::getHostURL().Yii::app()->createUrl("mobileapp/cron/processpush"));
		    	}
	    	}
   ...
   public static function MerchantchangeStatus($mtid='' , $data='')
   {
	   ...
	   $pattern=array(		    	   
    	...
    	$tpl=FunctionsV4::replaceTemplateTags($tpl,$pattern,$data); 
    	$subject=FunctionsV4::replaceTemplateTags($subject,$pattern,$data); 
    	$tpl_sms=FunctionsV4::replaceTemplateTags($tpl_sms,$pattern,$data); 	
	...
	public static function sendInvoiceNotification($merchant_id='', $data='', $pdf_filename='')
	{
		...
		if ($is_enabled==true && !empty($merchant_email)){
			...
			$data['period']= Yii::t("default",'[from] to [to]',array(
			  '[from]'=>FunctionsV4::prettyDate($data['date_from']),
			  '[to]'=>FunctionsV4::prettyDate($data['date_to']),
			));
